...
class MetaLanguage(object):
    ...
    def __init__(self, preds, metapreds, funcs, consts, metaconsts):
        self.preds = preds
        self.metapreds = metapreds
        self.funcs = funcs
        self.consts = consts
        self.metaconsts = metaconsts
    ...
    def __str__(self):
        ...
        for const in self.consts:
            s += const.__str__() + '\n'
        s += "===MetaConstants===\n"
        s += ("too much, not show, there are totally " + str(len(self.metaconsts))
              + " metaconstants, \n and you can modify in language.py class Metalanguage\n")
        # for metaconst in self.metaconsts:
        #     s += metaconst.__str__() + ' ' + metaconst.dtype+ ' ' #+  str(metaconst.value) + str(type(metaconst.value))
        return s
    ...
    def get_meta_by_dtype(self, dtype):

        return [c for c in self.metaconsts if c.dtype == dtype]
    ...
    def get_meta_pred_by_name(self, meta_pred_name):
        pred = [pred for pred in self.metapreds if pred.name == meta_pred_name]
        #assert len(pred) == 1, 'Too many or less match in ' + meta_pred_name
        return pred[0]
